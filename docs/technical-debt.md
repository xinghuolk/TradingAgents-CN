...
| Medium | Frontend lint and format | Lint configuration references a missing ignore file and undeclared patch package; Prettier reports 103 files with drift. | Existing commands are not deterministic and broad auto-fix creates noisy changes. | First repair lint invocation; then clean only touched areas until the diagnostic is green. |
